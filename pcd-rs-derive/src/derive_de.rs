...
pub fn f_pcd_record_read_derive(item: ItemStruct) -> syn::Result<TokenStream> {
    ...
    let expanded = quote! {
        impl ::bye_pcd_rs::record::PcdDeserialize for #struct_name {
            fn is_dynamic() -> bool {
                false
            }

            fn read_spec() -> Vec<(Option<String>, ::bye_pcd_rs::metas::ValueKind, Option<usize>)> {
                #read_spec_tokens
            }

           fn read_chunk<R: std::io::BufRead>(reader: &mut R, field_defs: &::bye_pcd_rs::metas::Schema) -> ::bye_pcd_rs::Result<#struct_name> {
                use ::bye_pcd_rs::byteorder::{LittleEndian, ReadBytesExt};
                let result = { #bin_read_tokens };
                Ok(result)
            }

            fn read_line<R: std::io::BufRead>(reader: &mut R, field_defs: &::bye_pcd_rs::metas::Schema) -> ::bye_pcd_rs::Result<#struct_name> {
    ...
    let bin_read_tokens = quote! {
        let #var_ident = {
            ...
            (0..count)
                ...
                .collect::<::bye_pcd_rs::Result<Vec<_>>>()?
        };
    };
    let text_read_tokens = quote! {
        let #var_ident = {
