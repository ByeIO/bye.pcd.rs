...
pub fn f_pcd_record_write_derive(item: ItemStruct) -> syn::Result<TokenStream> {
    ...
    let expanded = quote! {
        impl ::bye_pcd_rs::record::PcdSerialize for #struct_name {
            fn is_dynamic() -> bool {
                false
            }

            fn write_spec() -> ::bye_pcd_rs::metas::Schema {
                #write_spec_tokens
            }

            fn write_chunk<R: std::io::Write>(&self, writer: &mut R, _: &::bye_pcd_rs::metas::Schema) -> ::bye_pcd_rs::Result<()> {
                use ::bye_pcd_rs::byteorder::{LittleEndian, WriteBytesExt};
                { #bin_write_tokens };
                Ok(())
            }

            fn write_line<R: std::io::Write>(&self, writer: &mut R, _: &::bye_pcd_rs::metas::Schema) -> ::bye_pcd_rs::Result<()> {
...
fn derive_named_fields(
    ...
    let write_spec_tokens = quote! {
        vec![#(#write_specs),*]
            .into_iter()
            .collect::<::bye_pcd_rs::metas::Schema>()
    };
